...
import MessageInput, { MentionModel, MessageInputContext, hasEffectiveMention } from "../MessageInput";
...
import { VideoCell } from "../../Messages/Video";
import { VoiceContent } from "../../Messages/Voice";
import {
  trySendImageFromPastedPreviewUrl,
  trySendVideoFromPastedPreviewPath,
...
export class Conversation extends Component<ConversationProps> implements ConversationContext {
    ...
    private _dragFileCallbacks: Array<(file: File) => boolean> = []
    private _multiSelectEscapeHandler?: (e: KeyboardEvent) => void
    ...
    fowardMessageUI(message: Message): void {
        WKApp.shared.baseContext.showConversationSelect((channels: Channel[]) => {
            let cloneContent: MessageContent = message.content;
            if (message.contentType === MessageContentTypeConst.voice) {
                const origin = message.content as unknown as VoiceContent;
                const v = new VoiceContent();
                v.url = origin.url;
                v.timeTrad = origin.timeTrad;
                v.waveform = origin.waveform;
                cloneContent = v;
            }
    ...
    addDragFileCallback(f: (file: File) => boolean): void {
        this._dragFileCallbacks.push(f)
    }
    ...
    componentDidMount() {
        ...
        this._multiSelectEscapeHandler = (e: KeyboardEvent) => {
            if (e.key !== "Escape") {
                return
            }
            if (!this.vm?.editOn) {
        ...
        window.addEventListener("keydown", this._multiSelectEscapeHandler)
    }


    componentWillUnmount() {
        if (this._multiSelectEscapeHandler) {
            window.removeEventListener("keydown", this._multiSelectEscapeHandler)
            this._multiSelectEscapeHandler = undefined
        }
        this.dealloc()
    }
    dealloc() {
    ...
    messageUI(message: MessageWrap, last: boolean) {
        ...
        if (message.revoke) {
            ...
            if (MessageCell && content?.realContentType === MessageContentTypeConst.smallVideo) {
                MessageCell = VideoCell
            } else if (MessageCell && message.contentType === MessageContentType.unknown && content?.contentObj && typeof content.contentObj?.url !== "undefined" && typeof content.contentObj?.cover !== "undefined") {
                MessageCell = VideoCell
            }
        }
    ...
    updateReminderDoneIfNeed(viewport: HTMLElement | null) {
        ...
        let change = false
        const newDoneIDs = new Array<number>()
        for (const reminder of reminders) {
            ...
            if (message && this.isVisiableMessage(message.message, viewport)) {
                reminder.done = true
                newDoneIDs.push(reminder.reminderID)
                change = true
                continue
            }
        }
        if (change) {
            ...
            if (newDoneIDs.length > 0) {
                WKSDK.shared().reminderManager.done(newDoneIDs)
            }
        }
        ...
        return <Provider create={() => {
            ...
            return <>
                <div className={classNames("wk-conversation", vm.fileDragEnter ? "wk-conversation-dragover" : undefined, vm.currentReplyMessage ? "wk-conversation-hasreply" : undefined)} style={{ "background": chatBg ? `url(${chatBg}) rgb(245, 247, 249)` : undefined }}>
                        ...
                        <div className="wk-conversation-messages" id={vm.messageContainerId} onScroll={this.handleScroll.bind(this)}>
...

                            {
                                vm.fileDragEnter ? <div className="wk-conversation-content-fileupload-mask" onDragOver={(event) => {
                                    ...
                                    for (const cb of this._dragFileCallbacks) {
                                        if (cb(file)) break
                                    }
                                }}>
                    ...
                    <div className="wk-conversation-footer">
                        <div className="wk-conversation-footer-content">
                            ...
                            <MessageInput members={this.vm.subscribers.filter((s) => s.uid !== WKApp.loginInfo.uid)} onContext={(ctx) => {
                                ...
                                // 回复/编辑 或 有效 @ 时，不走「整段粘贴当预览链发图/视频」
                                if (!vm.currentReplyMessage && !hasEffectiveMention(mention)) {
                                    try {
                                        const sentAsImage = await trySendImageFromPastedPreviewUrl(
                                            (c) => this.sendMessage(c),
                                            text
                                        );
                                        if (sentAsImage) {
                                            return;
                                        }
                                        const sentAsVideo = await trySendVideoFromPastedPreviewPath(
                                            (c) => this.sendMessage(c),
                                            text
                                        );
                                        if (sentAsVideo) {
                                        ...
                                        Toast.error(err?.message || err?.msg || "无法从链接发送图片或视频");
                                        return;
                                    }
                                }
                                const content = new MessageText(text)
                                if (hasEffectiveMention(mention)) {
                                ...
                                if (vm.currentReplyMessage) {
                                    if (vm.currentHandlerType === 2) {
                                        ...
                                        try {
                                            ...
                                            vm.currentReplyMessage = undefined
                                            vm.currentHandlerType = 0
                                        } catch (err: any) {
                                            Toast.error(err?.msg || "编辑失败")
                                        }
                                        return
                                    }
                    ...
                    {/* 多选条放在 footer 之后，避免输入区叠层挡住关闭按钮与操作 */}
...
class MultiplePanel extends Component<MultiplePanelProps> {
    ...
    render(): React.ReactNode {
        ...
        return <div className="wk-multiplepanel">
            <div className="wk-multiplepanel-close" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => {
                e.stopPropagation()
                onClose?.()
            }}>
