...
export default class ConversationVM extends ProviderListener {
    ...
    async editMessage(messageID: String, messageSeq: number, channelID: String, channelType: number, content: String): Promise<void> {
        ...
        const extras = await WKSDK.shared().chatManager.syncMessageExtras(this.channel, this.findMaxExtraVersion())
        this.updateMessageByMessageExtras(extras)
    }
    ...
    didMount(): void {
        ...
        this.cmdListener = (message: Message) => {
            ...
            if (cmdContent.cmd === 'messageRevoke') { //消息撤回
                ...
                const matchByMessage = message.channel && message.channel.isEqual(this.channel)
                const matchByParam = param?.channel_id && new Channel(param.channel_id, param.channel_type).isEqual(this.channel)
                if (matchByMessage || matchByParam) {
