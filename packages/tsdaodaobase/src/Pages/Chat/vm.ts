...
export class ChatVM extends ProviderListener {
    ...
    setConnectTitleWithConnectStatus(connectStatus: ConnectStatus) {
        ...
        } else if (connectStatus === ConnectStatus.ConnectFail) {
            // SDK 在 CONNACK 失败时置为 ConnectFail 且不再重连；此前误显示为「连接中」
            this.connectTitle = "连接失败"
        } else if (connectStatus === ConnectStatus.ConnectKick) {
            this.connectTitle = "已下线"
        } else {
