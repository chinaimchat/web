...
import {
  ...
  copyMessageMediaLink,
  getMessageMediaAbsoluteUrl,
      ...
      return (
        <IconClick
          ...
          onClick={() => {
            ...
            if (win.__POWERED_ELECTRON__ && win.ipc?.send) {
              try {
                win.ipc.send("screenshots-start", {});
              } catch (e) {
                window.open("https://www.snipaste.com");
                Toast.info("未检测到截图组件，已打开 Snipaste 官网");
              }
            } else {
              window.open("https://www.snipaste.com");
              Toast.info("浏览器环境下请使用系统截图或 Snipaste 等工具");
            }
          }}
        ></IconClick>
      );
  ...
  registerMessageContextMenus() {
    ...
    WKApp.endpoints.registerMessageContextMenus(
      ...
      (message) => {
        ...
        if (
          ...
          message.contentType !== MessageContentTypeConst.gif
        ) {
          return null;
        }
        const c = message.content as { url?: string; imgData?: string };
        if (!c.url && !(c.imgData && String(c.imgData).startsWith("data:image"))) {
          return null;
        }
        return {
          title: "复制图片",
          onClick: () => {
            copyMessageImageToClipboard(message)
              .then(() => Toast.success("图片已复制"))
              .catch((err: any) => {
                Toast.error(err?.message || err?.msg || "复制图片失败");
              });
          },
        };
      },
    ...
    WKApp.endpoints.registerMessageContextMenus(
      ...
      (message) => {
        ...
        if (!getMessageMediaAbsoluteUrl(message)) {
          return null;
        }
        return {
          title: "复制媒体",
          onClick: () => {
            copyMessageMediaLink(message)
              .then(() => Toast.success("已复制"))
              .catch((err: any) => {
                Toast.error(err?.message || err?.msg || "复制失败");
              });
          },
        };
      },
    ...
    WKApp.endpoints.registerMessageContextMenus(
      "contextmenus.edit",
      (message, context) => {
        if (message.messageID === "" || !message.send) {
          return null;
        }
        if (message.remoteExtra?.revoke) {
          return null;
        }
        if (!WKApp.shared.supportEdit.includes(message.contentType)) {
          return null;
        }
        return {
          title: "编辑",
  ...
  registerUserInfo() {
    WKApp.shared.userInfoRegister(
      ...
      (context: RouteContext<UserInfoRouteData>) => {
        ...
        // 进群方式：须与后端 users/:uid?group_no= 一致。仅特权号接口会返回 join_group_* / group_member.invite_uid 等；
        // 勿用 IM Subscriber.orgData，否则非特权仍可能看到缓存中的邀请信息。
        const org = channelInfo?.orgData as any;
        const gm = org?.group_member;
        const inviteUid = String(
          org?.join_group_invite_uid ?? gm?.invite_uid ?? ""
        ).trim();
        const joinTimeRaw = String(
          org?.join_group_time ?? gm?.created_at ?? ""
        ).trim();
        const hasJoinPrivilegeMeta = Boolean(inviteUid || joinTimeRaw);
        if (fromSubscriberOfUser && hasJoinPrivilegeMeta) {
          let joinDesc = joinTimeRaw ? joinTimeRaw.substring(0, 10) : "";
          if (inviteUid) {
            const inviterChannel = new Channel(inviteUid, ChannelTypePerson);
