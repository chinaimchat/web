...
/** 拼接 API 根与资源 path，避免 `.../v1/` + `/files/...` 或缺少 `/` 在部分环境下请求失败（Electron/反代常见） */
...
export class ChannelDataSource implements IChannelDataSource {
    ...
    /** 解散群（仅群主可调用，对应服务端 DELETE /groups/:group_no/disband） */
    disband(channel: Channel): Promise<void> {
        if (channel.channelType !== ChannelTypeGroup) return Promise.resolve()
...
export class CommonDataSource implements ICommonDataSource {
    ...
    getFavoritesAll(): Promise<any> {
        return WKApp.apiClient.get("favorite/my", { param: { page_index: 1, page_size: 10000 } });
    }
    ...
    */
    getImageURL(path: string, opts?: { width: number, height: number }): string {
        ...
        return joinApiResourceUrl(baseURl, path)
    }
