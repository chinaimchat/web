...
export default class MainVM extends ProviderListener {
  ...
  didMount(): void {
    if (WKApp.route.currentPath) {
      ...
      if (!this._currentMenus && WKApp.route.currentPath === "/sticker") {
        this.pushRoutePath("/sticker");
      }
    }
  ...
  /** 程序化进入某路由（如 /sticker 无侧栏菜单时由「表情包商店」按钮调用） */
  pushRoutePath(routePath: string) {
    if (this._historyRoutePaths.indexOf(routePath) === -1) {
      this._historyRoutePaths.push(routePath);
    }
    this.setCurrentMenusByRoutePath(routePath);
    this.notifyListener();
  }

  /** 从左侧历史栈弹出当前页（如表情包商店点击返回时调用） */
  popRoutePath() {
    if (this._historyRoutePaths.length <= 1) {
      // 若当前只剩 /sticker 这类“无侧栏菜单页面”，关闭时需要回到一个正常菜单页，
      // 否则浏览器地址会一直停在 /sticker，刷新后仍回到商店页。
      const fallback = this.menusList[0]?.routePath || "/";
      this._historyRoutePaths = [fallback];
      this.setCurrentMenusByRoutePath(fallback);
      this.notifyListener();
      return;
    }
    this._historyRoutePaths.pop();
    const last = this._historyRoutePaths[this._historyRoutePaths.length - 1];
    this.setCurrentMenusByRoutePath(last);
    this.notifyListener();
  }

  private setCurrentMenusByRoutePath(routePath: string) {
    for (const menus of this.menusList) {
      if (menus.routePath === routePath) {
        this._currentMenus = menus;
        return;
      }
    }
    const emptyIcon = React.createElement("span");
    this._currentMenus = new Menus("_route_" + routePath, routePath, routePath === "/sticker" ? "表情包商店" : routePath, emptyIcon, emptyIcon);
  }
